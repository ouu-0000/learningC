...
|double | 小数(floatの二倍のビットを使って表現できるのでほぼこっちを使う) | %lf|
...
## if文連打が見やすくなるswitch文  
もしも変数aの値によって実行する処理を変えたい場合、if文を使おうとすると以下のようになります。  
```c
#include<stdio.h>
int main(){
  int a;
...
int main(){
  ...
  switch(a){
    case 0:  //aの値が0なら実行される
      printf("%dの示す抵抗のカラーコードは黒です。\n", a);
      break;  //switchを抜けるもし書いていないと、この下のcase 1:の処理に進む
    case 1:
      printf("%dの示す抵抗のカラーコードは茶です。\n", a);
      break;
    case 2:
...
ここで`break;`について解説していきます。  
switch文の中でbreakを使うとswitchを出ますが、while文や、for文などの繰り返し文の中で呼び出すとbreakのあとの処理を行わずに繰り返し文を出ることができます。  
制御式とは違う条件で繰り返しをやめたい時とかに使う。  
## 繰り返しはwhile文  
...
# 番外編  
## 表示したものを全部消す  
## キーボードのあるキーが押されたのを検知する  
# 勉強になるサイト  
C言語の解説をしているサイト。わかりやすい。  
[苦しんで覚えるC言語](https://9cguide.appspot.com/s_start.html)  
読みやすいプログラムについての解説  
[初心者にもわかりやすくリーダブルコードについてまとめてみた](https://wayasblog.com/readable-code/)  
